 ...
 *
 * <p>Esta clase implementa la ventana principal de la aplicación utilizando
 * Swing. Permite visualizar los ejércitos, gestionar personajes, configurar
 * opciones de juego (música, mapas) y ejecutar batallas con animación de texto.</p>
 *
...
public class MainGUI extends JFrame {
	...
	public MainGUI() {
		...
		// Cambiar logotipo
		URL iconURL = getClass().getClassLoader().getResource("fondos/logo.jpg");
		if (iconURL != null) {
	...
	private void abrirVentanaMapas() {
	    ...
	    try {
	        URL rutaMapa = dialogo.getRutaMapaSeleccionado();
	        if (rutaMapa != null) {
	            aplicarMapaDeFondo(rutaMapa.toString());
	            rutaMapaActual = rutaMapa.toString();
	        }
	    } catch (Exception e) {
	...
	private void cargarMapaPorDefecto() {
	    try {
	        URL rutaMapaDefecto = getClass().getClassLoader().getResource("fondos/default.jpg");
	        if (rutaMapaDefecto != null) {
	            aplicarMapaDeFondo(rutaMapaDefecto.toString());
	            rutaMapaActual = rutaMapaDefecto.toString();
	        }
	    } catch (Exception e) {
	...
	/**
     * Escribe texto en el área de log con un efecto de máquina de escribir.
     *
     * @param texto El contenido textual a animar.
     */
